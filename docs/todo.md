...
- **完成说明**: 已实现——维护 `PendingWrites` 覆盖层（`None` = 挂起删除），写路径统一收敛到 `stage_upsert_kv`（同时写 WriteBatch 和覆盖层），Txn 条件/prev_values 经 `get_kv_with_overlay()` 读取。新增 3 个单测覆盖同批次 Txn 链、挂起删除可见、分歧场景。
